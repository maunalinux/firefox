...
if not abort:
    ...
    os.system("cp %s/pref/distribution.ini %s/debian/firefox/opt/firefox/distribution" % (curdir, curdir))
...
for xpi in codes.keys():
    ...
    os.system("mkdir -p %s/debian/%s-%s/opt/firefox/distribution/extensions" % (curdir, locale_prefix, package_code))
    os.chdir("%s/debian/%s-%s/opt/firefox/distribution/extensions" % (curdir, locale_prefix, package_code))
